...
@export var hero_traits_fallback: Array[TraitResource] = []
@export var enemy_traits_fallback: Array[TraitResource] = []
...
func load_all() -> void:
	...
	if hero_traits.is_empty() and not hero_traits_fallback.is_empty():
		push_warning("[TraitDatabase] Hero traits folder vacia, usando fallback.")
		hero_traits = hero_traits_fallback.duplicate()
	if enemy_traits.is_empty() and not enemy_traits_fallback.is_empty():
		push_warning("[TraitDatabase] Enemy traits folder vacia, usando fallback.")
		enemy_traits = enemy_traits_fallback.duplicate()
