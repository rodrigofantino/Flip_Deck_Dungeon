...
func _build_stats_rows() -> void:
	...
	for stat: int in HeroUpgradeStats.get_all_stats():
		...
		name_label.text = HeroUpgradeStats.get_stat_name(stat)
		name_label.tooltip_text = HeroUpgradeStats.get_stat_desc(stat)
...
func _refresh_selected_hero() -> void:
	...
	xp_label.text = "XP: %d / %d" % [progression.xp, xp_required]
	points_label.text = "Unspent: %d" % progression.unspent_points
